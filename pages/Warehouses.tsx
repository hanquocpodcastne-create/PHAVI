                        ...
                        {warehouses.map(w => (
                            <tr key={w.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                ...
                                <td className="py-4 px-6 flex items-center space-x-3">
                                    <button onClick={() => openModal(w)} className="text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-600" title="Sửa">
                                        <EditIcon className="w-5 h-5"/>
                                    </button>
                                    <button onClick={() => handleDelete(w.id)} className="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-600" title="Xóa">
                ...
                <Modal isOpen={isModalOpen} onClose={closeModal} title={'id' in editingWarehouse ? 'Chỉnh sửa Kho hàng' : 'Thêm Kho hàng mới'}>
                    <div className="space-y-4">
                        <div>
                            ...
                            <input type="text" name="name" value={editingWarehouse.name} onChange={handleInputChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Địa điểm</label>
                            <input type="text" name="location" value={editingWarehouse.location} onChange={handleInputChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                        </div>
                    </div>
